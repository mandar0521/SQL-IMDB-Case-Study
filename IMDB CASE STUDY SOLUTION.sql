...
-- Segment 1:
-- code to use the schema, in this case it is imdb
USE imdb;
...
-- We can see that 4 columns: country, worlwide_gross_income, languages, and production_company have null values.
...
-- We can see that 2017 was the year with highest movie production
-- We can also see that most of movies are produced in the month of March
...
-- There are 3289 movies with single genre
...
-- Kirket and Love in Kilnerry top the list of highest avg_rating list
...
-- The movie Fan is present in the top 10 movies in the code above.
...
-- Majority of the movies fall between median_rating of 6-8, while median rating of 7 is the highest
...
-- Two production companies Dream Warrior Pictures and National Theatre Live have produced most hit movies (avg rating > 8) i.e 3 each
...
-- In the March of 2017 most number of movies were produced in the genre Drama in USA
...
-- There are 361 movies producted between 1st April 2018 and 1st April 2019 with median rating of 8.
...
-- We can see that 146 German movies have got votes, while only 123 Italian movies have got votes. 
-- So yes, German movies get more votes than Italian movies
...
-- We can see that James Mangold is the director with most highest rated movies.
...
-- Actor Mammootty and Mohanlal are the top 2 rated actors whose movies have median_ratings of >= 8
...
-- Marvel Studios along with Twentieth Century Fox and Warner Bros tops the list of production houses based on number of votes received
...
-- Taapsee Pannu is at the top of the list
...
-- Top 3 Genres are Drama, Comedy and Thriller based on most bumber of movies
...
-- Star Cinema and Twentieth Century Fox are top two production houses that have produced the highest number of hits among multilingual movies.
...
-- Parvathy Thiruvothu tops the list
